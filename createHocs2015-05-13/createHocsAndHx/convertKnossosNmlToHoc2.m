function y=convertKnossosNmlToHoc2(a,filename,overWriteEdges,overWriteThickness,emphasizeNodes,useSplines,resolution,oxalisdiameter)

for i=1:size(a,2)
    if size(a{i}.nodes,1)>0
...
%             y=interp1(xx,yy,x);
            ...
            diamfactor=mean(resolution);
            if oxalisdiameter
                diamfactor=1;
            end
            
            for jj=listS{ii,1}'
                fprintf(fid,'{pt3dadd(%f,%f,%f,%f)}\n',jj(2)*resolution(1),jj(3)*resolution(2),jj(4)*resolution(3),jj(5)*diamfactor);
            end
        end
        fclose(fid);
    end
